...
public static class AedTests
{
    public static async Task Run(HttpClient client)
    {
        ...
        // GET all AED points
        var points = await client.GetFromJsonAsync<List<AedDto>>("api/aed");
        Console.WriteLine($"GET /api/aed -> {points?.Count} punktów AED");

        // Przykładowy POST (dodanie punktu)
        var newPoint = new AedDto { Latitude = 50.0m, Longitude = 20.0m, Description = "Test AED", Verified = false };
